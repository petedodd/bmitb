...
lopoff17 <- function(x) ifelse(x<17,0,
...
RRlopoff <- function(k,theta,t1,t2,L) RRlopoff0(k,theta,t1,t2,L) /
...
## by age and sex
RRbyAS <- DRB[Year==2022,.(RR0=weighted.mean(RR0,tb),
                 RR17=weighted.mean(RR17,tb),
                 RR18.5=weighted.mean(RR18.5,tb)),by=.(Sex,age)]
...
## by age and sex
RRbyASR <- DRB[Year==2022,.(RR0=weighted.mean(RR0,tb),
                           RR17=weighted.mean(RR17,tb),
                           RR18.5=weighted.mean(RR18.5,tb)),by=.(Sex,age,g_whoregion)]
...
## by region and sex
RRbySR <- DRB[Year==2022,.(RR0=weighted.mean(RR0,tb),
                           RR17=weighted.mean(RR17,tb),
                           RR18.5=weighted.mean(RR18.5,tb)),by=.(Sex,g_whoregion)]
RRbySR[,RR17:=RR17/RR0]
RRbySR[,RR18.5:=RR18.5/RR0]
RRbySR <- melt(RRbySR[,.(Sex,g_whoregion,RR17,RR18.5)],id=c("Sex","g_whoregion"))

## sex globall
RRbySG <- DRB[Year==2022,.(RR0=weighted.mean(RR0,tb),
                           RR17=weighted.mean(RR17,tb),
                           RR18.5=weighted.mean(RR18.5,tb)),by=.(Sex)]
...
## country lopoffs
RRbyC <- DRB[Year==2022,.(RR0=weighted.mean(RR0,tb),
                          RR17=weighted.mean(RR17,tb),
                          RR18.5=weighted.mean(RR18.5,tb),
                          tb=sum(tb)),
             by=.(iso3,g_whoregion)]
